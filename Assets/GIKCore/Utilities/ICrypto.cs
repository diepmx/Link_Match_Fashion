...
namespace GIKCore.Utilities
{
    public class ICrypto
    {
        ...
        public static byte[] CompressStringToBrotli(byte[] data)
        {
            using (MemoryStream memoryStream = new MemoryStream())
            {
                using (BrotliStream brotliStream = new BrotliStream(memoryStream, CompressionLevel.Optimal, true))
                {
        ...
        public static byte[] CompressStringToBrotli2(string s)
        {
            byte[] data = Encoding.UTF8.GetBytes(s);
            return CompressStringToBrotli(data);
        }
        public static string DecompressBrotliToString(byte[] compressedData)
        {
            using (MemoryStream memoryStream = new MemoryStream(compressedData))
            {
                ...
                {
                    using (StreamReader reader = new StreamReader(brotliStream, Encoding.UTF8))
                    {
